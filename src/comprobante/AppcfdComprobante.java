...
public class AppcfdComprobante implements Runnable {
    ...
    BufferedInputStream in_certificado, in_certificado64, in_llave, in_image;
    File cert, llave, image;
    ...
    com.codinet.facture.cfdv2.Comprobante.Impuestos.
    Retenciones impuestosRetenciones;
    com.codinet.facture.cfdv2.Comprobante.Impuestos.
    ...
    Thread th;
    String host, bd, us, pss, d, sql, msg, nombre, rfc, calle, colonia, cp,
    		interior, exterior, estados, delegacion, pais, emi, recep,
    		descripcion, concep, ncertificado, sello, version, timpuesto,
    		rimpuesto, comprob, nombre_empresa, nombre_sucursal, prefijo, pagge,
    		cad_original, sello_generado, pwdLlave, certificado, xmlRutaDestino,
    		com[], folio, serie, rfc_emisor, pass, direccion;
    Blob pubkey, prikey, im;
    Date fecha;
    double cantidad, subtotal, descuento, total_impuestos, importe,
    		valor_unitario;
    long idEntidadEmisor, idEntidadReceptor, anio, naprovacion, idEmpresa, 
    		position, idempresa, sucursal;
    int len, pagos, plazos;
    byte bytes[];
    InputStream blobInputStream, isLlavePrivada, isLlavePublica;
    BufferedInputStream in;
    BufferedOutputStream out;
    ResultSet rs, rs2, rs_company, rs_val;
    Properties myProp;
    ...
    public void setParams(long parm1, String parm2, String parm3, long parm4)
        ...
        serie = parm3;
        sucursal = parm4;
        vf = new ValidaFolio(Getidfolio(Long.toString(parm1), parm3),
    ...
    public String Getidfolio(String empresa, String serie) throws SQLException {
        if (serie.equals("_")) {
            sql = (new StringBuilder("SELECT * FROM cfd_folios WHERE idEmpre" +
            		"sa = ")).append(empresa).append(" AND ").append("serie " +
            				"is null or serie = '' ").toString();
            rs = cnx.consulta(sql, true);
        } else {
            sql = (new StringBuilder("SELECT * FROM cfd_folios WHERE idEmpre" +
            		"sa = ")).append(empresa).append(" AND ").append("serie " +
            				"= '").append(serie).append("'").toString();
            rs = cnx.consulta(sql, true);
        }
        if (rs.next()) {
    ...
    public void creaConfiguracion() throws SQLException, IOException {
        ...
        sql = (new StringBuilder("SELECT * FROM cfd_config  WHERE cfd_config." +
        		"idEmpresa =")).append(idempresa).append(" AND ").append("cfd" +
        				"_config.idSucursal = ").append(sucursal).toString();
        ...
        try {
            if (rs.next()) {
                ...
                in_certificado = new BufferedInputStream(
                									pubkey.getBinaryStream());
                in_certificado64 = new BufferedInputStream(
                									pubkey.getBinaryStream());
                ...
                java.security.cert.X509Certificate x509certificate = 
                	firmasdigitales.cargaCert(new BufferedInputStream(
                			pubkey.getBinaryStream()));
                if (x509certificate != null) {
                    if (!firmasdigitales.certNumSerie(x509certificate).
                        ...
                        System.out.println((new StringBuilder("Numero de seri" +
                        		"e incorrecto: ")).append(firmasdigitales.
                        		certNumSerie(x509certificate)).append(" vs ").
                        		append(rs.getString("seriecertificado")).
                        		toString());
                    } else {
                        System.out.println((new StringBuilder("Numero de seri" +
                        		"e correcto: ")).append(firmasdigitales.
                        		certNumSerie(x509certificate)).append(" vs ").
                        		append(rs.getString("seriecertificado")).
                        		toString());
                    }
    ...
    public void creaEmisor() throws SQLException, Exception {
        ...
        sql = (new StringBuilder("SELECT  emisor,receptor,idEmpresa,EXTRACT" +
        		"(YEAR FROM fecha) as anio,noAprobacion,noCertificado,sello" +
        		",vers FROM cfd_comprobante  WHERE folio = ")).
        		append(folio).append(" and serie = '").append(serie).append(
        ...
        sql = (new StringBuilder("SELECT cnet_entidades.nombre,cnet_entidade" +
        		"s.rfc,cnet_domicilios.calle,cnet_colonias.codigo_postal,cne" +
        		"t_colonias.descripcion as colonia,cnet_estados.descripcion " +
        		"as estado,cnet_delegacion.nombre as delegacion,cnet_domicil" +
        		"ios.n_exterior,cnet_domicilios.n_interior,cnet_pais.nombre " +
        		"as nombrepais,cnet_estados.pais FROM cnet_colonias,cnet_del" +
        		"egacion,cnet_domicilios,cnet_entidades,cnet_pais,cnet_estad" +
        		"os WHERE ( cnet_entidades.idEntidad = cnet_domicilios.idEnt" +
        		"idad ) and( cnet_domicilios.idEstado = cnet_estados.idEstad" +
        		"o ) and( cnet_domicilios.idColonia = cnet_colonias.idColoni" +
        		"a ) and( cnet_colonias.idDelegacion = cnet_delegacion.idDel" +
        		"egacion ) and( cnet_estados.pais = cnet_pais.id ) and( ( cn" +
        		"et_entidades.idEntidad =")).append(idEntidadEmisor).append(
        		"))").toString();
        rs = cnx.consulta(sql, true);
        if (rs.next()) {
            ...
            emisor = ofactory.createComprobanteEmisor(new TUbicacionFiscal(rs.
            		getString("calle"), rs.getString("delegacion"), rs.
            		getString("estado"), rs.getString("nombrepais"), cp), rs.
            		getString("nombre"), rfc_emisor);
    ...
    public void creaReceptor() throws SQLException, Exception {
        ...
        sql = (new StringBuilder("SELECT cnet_entidades.nombre,cnet_entidades" +
        		".rfc,cnet_domicilios.calle,cnet_colonias.codigo_postal,cnet_" +
        		"colonias.descripcion as colonia,cnet_estados.descripcion as " +
        		"estado,cnet_delegacion.nombre as delegacion,cnet_domicilios." +
        		"n_exterior,cnet_domicilios.n_interior,cnet_pais.nombre as no" +
        		"mbrepais,cnet_estados.pais FROM cnet_colonias,cnet_delegacio" +
        		"n,cnet_domicilios,cnet_entidades,cnet_pais,cnet_estados WHER" +
        		"E ( cnet_entidades.idEntidad = cnet_domicilios.idEntidad ) a" +
        		"nd( cnet_domicilios.idEstado = cnet_estados.idEstado ) and( " +
        		"cnet_domicilios.idColonia = cnet_colonias.idColonia ) and( c" +
        		"net_colonias.idDelegacion = cnet_delegacion.idDelegacion ) a" +
        		"nd( cnet_estados.pais = cnet_pais.id ) and( ( cnet_entidades" +
        		".idEntidad =")).append(idEntidadReceptor).append("))").
        		toString();
        rs = cnx.consulta(sql, true);
        if (rs.next())
        {
            receptor = ofactory.createComprobanteReceptor(rs.getString("rfc"),
            					rs.getString("nombre"),
            					new TUbicacion(rs.getString("calle")));
            ...
            receptor.getDomicilio().setCodigoPostal(rs.getString("codigo" +
            		"											_postal"));
            ...
            if (!rs.getString("n_interior").equals("0")) {
                receptor.getDomicilio().setNoInterior(rs.getString("n_in" +
                												"terior"));
            }
        }
    }
    ...
    public void creaLugarExpedicion() throws Exception {
        ...
        sql = (new StringBuilder("SELECT cnet_entidades.nombre,cnet_estados." +
        		"pais,cnet_colonias.codigo_postal,cnet_colonias.descripcion " +
        		"as colonia,cnet_estados.descripcion as estado,cnet_delegaci" +
        		"on.nombre as delegacion,cnet_domicilios.calle,cnet_domicili" +
        		"os.n_exterior,cnet_pais.nombre as nombrepais,cnet_sucursal." +
        		"tipo,cnet_domicilios.n_interior FROM cnet_entidades,cnet_su" +
        		"cursal,cnet_domicilios,cnet_colonias,cnet_delegacion,cnet_p" +
        		"ais,cnet_estados WHERE ( cnet_sucursal.identidad = cnet_ent" +
        		"idades.idEntidad ) and( cnet_domicilios.identidad  = cnet_s" +
        		"ucursal.identidad ) and( cnet_domicilios.idDelegacion = cne" +
        		"t_delegacion.idDelegacion ) and( cnet_domicilios.idEstado =" +
        		" cnet_estados.idEstado ) and( cnet_domicilios.idColonia = c" +
        		"net_colonias.idColonia ) and( cnet_estados.pais = cnet_pais" +
        		".id ) and( ( cnet_sucursal.idsucursal = ")).append(sucursal).
        		append(") )").toString();
        rs = cnx.consulta(sql, true);
        if (rs.next()) {
    ...
    public void creaConceptos() throws Exception {
        ...
        sql = (new StringBuilder("SELECT cfd_conceptos.cantidad,cfd_concepto" +
        		"s.descripcion,cfd_conceptos.importe,cfd_conceptos.valorUnit" +
        		"ario FROM cfd_conceptos WHERE folio = ")).append(folio).
        		append(" and serie = '").append(serie).append("' and idEmpre" +
        		"sa = ").append(idempresa).toString();
        rs = cnx.consulta(sql, true);
        conceptos = ofactory.createComprobanteConceptos();
        while (rs.next()) {
            conceptos.getConcepto().add(ofactory.
            		createComprobanteConceptosConcepto(rs.getDouble("cantidad"),
            		rs.getString("descripcion"), rs.getDouble("importe"),
    ...
    public void creaImpuestos() throws SQLException {
        ...
        sql = (new StringBuilder("SELECT sum(cfd_impuestos.importe) as impor" +
        		"te,cnet_impuesto.descripcion,cfd_impuestos.idCNImpuesto,cfd" +
        		"_impuestos.idCatImpuesto FROM cfd_impuestos,cnet_impuesto W" +
        		"HERE ( cfd_impuestos.idCNImpuesto = cnet_impuesto.idCNImpue" +
        		"sto ) and( ( cfd_impuestos.folio = ")).append(folio).append(
        		" ) AND").append("( cfd_impuestos.serie = '").append(serie).
        		append("' ) AND").append("( cfd_impuestos.idEmpresa =").append(
        		idempresa).append(") AND").append("(( cfd_impuestos.tipo = '" +
        		"+' )or( cfd_impuestos.tipo = 'T' ))) group by cnet_impuesto" +
        		".descripcion,cfd_impuestos.idCNImpuesto,cfd_impuestos.idCat" +
        		"Impuesto").toString();
        rs = cnx.consulta(sql, true);
        do {
            if (!rs.next()) {
            ...
            sql = (new StringBuilder("SELECT cfd_det_impuestos.impuesto FROM" +
            		"   cfd_det_impuestos WHERE ( cfd_det_impuestos.idCNImpu" +
            		"esto = ")).append(rs.getInt("idCNImpuesto")).append(") " +
            		"and").append("( cfd_det_impuestos.idCatImpuesto =").
            		append(rs.getInt("idCatImpuesto")).append(")").toString();
            rs2 = cnx.consulta(sql, true);
            if (rs2.next()) {
                if (rs.getString("descripcion").trim().equals("IEPS")) {
                    impuestosTraslados.getTraslado().add(ofactory.
                    		createComprobanteImpuestosTrasladosTrasladoIEPS(
                    		Math.abs(rs.getDouble("importe")), rs2.getDouble(
                    		"impuesto")));
                }
        ...
        sql = (new StringBuilder("SELECT sum(cfd_impuestos.importe)as import" +
        		"e,cnet_impuesto.descripcion FROM cfd_impuestos,cnet_impuest" +
        		"o WHERE ( cfd_impuestos.idCNImpuesto = cnet_impuesto.idCNIm" +
        		"puesto ) and( ( cfd_impuestos.folio = ")).append(folio).
        		append(" ) AND").append("( cfd_impuestos.serie = '").append(
        		serie).append("' ) AND").append("( cfd_impuestos.idEmpresa =").
        		append(idempresa).append(") AND").append("(( cfd_impuestos.t" +
        				"ipo = '-' )or( cfd_impuestos.tipo = 'R' ))) group b" +
        				"y cnet_impuesto.descripcion,cfd_impuestos.idCNImpue" +
        				"sto,cfd_impuestos.idCatImpuesto").toString();
        rs = cnx.consulta(sql, true);
        do {
            if (!rs.next()) {
                break;
            ...
            if (rs.getString("descripcion").trim().equals("IVA")) {
                impuestosRetenciones.getRetencion().add(ofactory.
                		createComprobanteImpuestosRetencionesRetencionIVA(
                				Math.abs(rs.getDouble("importe"))));
            }
            if (rs.getString("descripcion").trim().equals("ISR")) {
                impuestosRetenciones.getRetencion().add(ofactory.
                		createComprobanteImpuestosRetencionesRetencionISR(
                				Math.abs(rs.getDouble("importe"))));
            }
        } while (true);
        if (rs.last()) {
        ...
        sql = (new StringBuilder("SELECT sum(cfd_impuestos.importe) as impor" +
        		"te  FROM cfd_impuestos,cnet_impuesto WHERE ( cfd_impuestos." +
        		"idCNImpuesto = cnet_impuesto.idCNImpuesto ) and( ( cfd_impu" +
        		"estos.folio = ")).append(folio).append(" ) AND").append("( " +
    ...
    public void creaObjetoComprobante() throws Exception {
        ...
        comprobante = ofactory.createComprobanteIngreso(emisor, receptor,
        		conceptos, impuestos, folio, new Date(), "SELLO", naprovacion,
        		anio, ncertificado, subtotal, subtotal + total_impuestos);
